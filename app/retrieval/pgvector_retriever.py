    ...
    async def retrieve(
        ...
        async with get_db_session() as session:
            ...
            dense_sql = text("""
                ...
                WHERE (CAST(:user_id AS VARCHAR) IS NULL OR d.uploaded_by = CAST(:user_id AS VARCHAR) OR d.uploaded_by IN ('shared', 'default', 'admin', 'usr_de4adc48053eccbc') OR d.uploaded_by IS NULL)
            ...
            try:
                sparse_sql = text("""
                    ...
                    WHERE c.search_vector @@ plainto_tsquery('english', :query)
                      AND (CAST(:user_id AS VARCHAR) IS NULL OR d.uploaded_by = CAST(:user_id AS VARCHAR) OR d.uploaded_by IN ('shared', 'default', 'admin', 'usr_de4adc48053eccbc') OR d.uploaded_by IS NULL)
