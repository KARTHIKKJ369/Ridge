...
* **No data-dump sections.** A 20-row publication table, a 30-row award list, a giant pricing matrix on a marketing page = wrong layout.
* **Long lists need a different UI component, not a longer list.** If you have > 5 items, reach for 2-column split with grouped items, card grid with image + label, tabs/accordion, horizontal scroll-snap pills, carousel, or marquee.
* **COPY SELF-AUDIT (mandatory before ship):** Before declaring any task done, re-read every visible string on the page. AI-generated cute copy is worse than boring copy.
* **Fake-precise numbers are flagged.** Numbers like `92%`, `4.1×`, `48k`, `5.8 mm`, `13.4 lb` must come from real data or be explicitly labeled as mock.
...
* The page has ONE theme. Sections do not invert mid-page.

---

## 5. CONTEXT-AWARE PROACTIVITY
* **Liquid Glass / Glassmorphism:** For premium consumer, Apple-adjacent, luxury brand, or media-overlay vibes.
* **Magnetic Micro-physics:** Use when `MOTION_INTENSITY > 5` with Motion's `useMotionValue` / `useTransform`.
* **Perpetual Micro-Interactions:** Apply Spring Physics (`type: "spring", stiffness: 100, damping: 20`).
* **MOTION MUST BE MOTIVATED (mandatory):** Each animation needs a clear communication purpose.
* **MARQUEE MAX-ONE-PER-PAGE (mandatory):** Appropriate at most ONCE per page.
...
* **NO generic names or startup-slop brand names.**
* **NO em-dash (`—`) as a design element OR anywhere else.**
* **NO version labels in hero.**
* **NO section-numbering eyebrows (`01 / INDEX`).**
* **NO decorative colored status dots on every item.**
* **NO div-based fake screenshots.**

---

## 14. FINAL PRE-FLIGHT CHECK
Run every verification item in the matrix before finalizing design tasks.
